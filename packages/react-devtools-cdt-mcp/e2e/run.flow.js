...
function parseTree(value: mixed): Array<TreeNode> {
  return expectArray(value, 'Expected component tree array').map(parseTreeNode);
}
...
async function runE2E(chrome: Chrome, appUrl: string): Promise<void> {
  ...
  const ownersStack = parseOwnersStack(
    await callTool('react_get_owners_stack', {uid: todo.uid})
  );
  ...
  const parentsBranch = parseComponentBranch(
    await callTool('react_get_parents_branch', {
      uid: todo.uid,
    }),
    'parents'
  );
  ...
  const ownersBranch = parseComponentBranch(
    await callTool('react_get_owners_branch', {
      uid: todo.uid,
    }),
    'owners'
  );
