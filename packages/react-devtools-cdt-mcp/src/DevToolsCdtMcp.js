...
const TOOL_DEFINITIONS: Array<ToolDefinition> = [
  {
    ...
    description:
      ...
      '{uid, type, name, key, firstChild, nextSibling}, where firstChild/' +
      'nextSibling link nodes by uid. uid is the stable handle the other ' +
      'tools accept. Covers every mounted root unless rootUid scopes the ' +
    ...
    call: (tools, args) => tools.getComponentTree(args.depth, args.rootUid),
  },
  {
    ...
    call: (tools, args) => tools.getComponentByUid(args.uid),
  },
  {
  ...
  },
  {
    ...
    call: (tools, args) => tools.getOwnersBranch(args.uid),
  },
  {
