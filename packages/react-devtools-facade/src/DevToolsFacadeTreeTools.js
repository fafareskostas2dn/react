...
export type TreeTools = {
  ...
  getParentsBranch: (uid: string) => Array<ParentEntry> | ToolError,
...
export function createTreeTools(
  ...
  function findByHostInstance(
    ...
    if (getHostInstanceForFiber(internals, fiber) === hostInstance) {
      return fiber;
    }
    const children = collectChildren(fiber);
    for (let i = 0; i < children.length; i++) {
      const found = findByHostInstance(internals, children[i], hostInstance);
      if (found != null) return found;
    }
    return null;
  }

  function buildNodeInfo(fiber: Fiber, internals: RendererInternals): NodeInfo {
    const info: NodeInfo = {
      uid: getUid(fiber),
      type: getTypeTagForFiber(internals, fiber),
      name: getDisplayName(internals, fiber),
  ...
  function getComponentByUid(uid: string): NodeInfo | ToolError {
    ...
    return buildNodeInfo(result.fiber, result.internals);
  }
   ...
   * @param uid - The component uid (e.g. "r5").
   */
  function getParentsBranch(uid: string): Array<ParentEntry> | ToolError {
    const result = findFiberByUid(uid);
    if (result.error != null) {
      return {error: result.error};
    }
   ...
   * mounted in the rendered tree. Use getParentsBranch for structural Fiber
